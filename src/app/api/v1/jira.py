...
async def get_jira_config():
    ...
    """
    return {"base_url": settings.JIRA_URL, "auth_mode": settings.JIRA_AUTH_MODE, "projects": settings.JIRA_PROJECTS}
...
async def search_jira_projects(query: str = "") -> list[dict[str, Any]]:
    ...
    params = {"query": query, "maxResults": 20, "action": "browse"}
...
async def get_jira_epics(project_key: str) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            payload = {"jql": jql, "maxResults": 50, "fields": ["summary", "description", "status", "labels"]}
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response = await client.get(
                url, params={"fields": "summary,description,status,issuetype,labels"}, auth=auth, timeout=10.0
            )
            response.raise_for_status()
            data = response.json()
