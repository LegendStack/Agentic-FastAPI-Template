...
from src.app.agents.demo import DemoAgentConfig, LegendDemoAgent
from src.app.agents.demo.mocks import MockGraphDB, MockLLM, MockVectorStore
...
class TestMockServices:
    ...
    async def test_mock_graph_db_merge(self):
        ...
        result = await graph.execute_query("MERGE (e:Person {name: $name})", {"name": "jane doe"})
    ...
    async def test_mock_graph_db_match(self):
        ...
        result = await graph.execute_query("MATCH (n {name: $name}) RETURN n", {"name": "legendstack"})
